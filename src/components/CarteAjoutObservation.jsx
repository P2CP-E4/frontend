    ...
    return (
        ...
        >
            <div className='flex flex-col w-full bg-white rounded-[45px] md:h-full'>
                {
                    status ? <SubmitCarte state={status} clear={clearStatus} titre={status === 'success' ? 'Ajout avec succès' : ' Echec dans l\'ajout'} message={status === 'success' ? "L'ajout de l'observation a été effectuée avec succès." : "Désolé , nous n'avons pas pu enregistrer votre Ajout veuillez réessayez plus tard, ou contacter l'administrateur pour obtenir de l'aide"} />
                        : <>
                            <Formik
