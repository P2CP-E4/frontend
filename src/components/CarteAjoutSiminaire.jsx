...
const CarteAjoutSiminaire = () => {
    ...
    const { handleSubmitForm, status, clearStatus } = useHandleSubmitFormAjouts(POST_SIMINAIRE_URL);
    ...
    return (
        ...
        >
            <div className='flex flex-col w-full py-4 px-5  bg-white rounded-[45px] md:h-full'>
                {
                    status ? <SubmitCarte state={status} clear={clearStatus} titre={status === 'success' ? 'Ajout avec success' : ' Echec dans l\'ajout'} message={status === 'success' ? "L'ajout du siminaire a été effectuée avec succées." : "Désolé , nous n'avons pas pu enregistrer votre inscription veuillez réessayez plus tard, ou contacter l'administrateur pour obtenir de l'aide"} />
                        : <>
