...
const FormsTextArea = ({ name, label, ...rest }) => {
    return (
        <>
            <label
                    ...
                    ({ form }) => {
                        return (
                            <div className='relative'>
                                <textarea
                                    className={'outline-none block w-full rounded-lg border-[#1C82AD] border py-2 px-4' + (form.touched[name] && form.errors[name] ? ' border-red-500' : '')}
                                    type="textarea"
                                    name={name}
                                    id={name}
